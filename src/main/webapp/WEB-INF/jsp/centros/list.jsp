		...
		<table class="table table-hover">
			...
			<c:forEach items="${centros}" var="centro">
				<tr>
					<td>${centro.id}</td>
					<td>${centro.nome}</td>
					<td>${centro.endereco.logradouro}, ${centro.endereco.numero}</td>
					<td>${centro.contato.telefone}</td>
					<td>${centro.contato.email}</td>
					<c:if test="${usuarioLogado.login eq 'admin'}">
					<td>
						<a href="executa?action=EditCentro&id=${centro.id}">
							<span class="glyphicon glyphicon-pencil"></span>
						</a>
						<a href="executa?action=RemoveCentro&id=${centro.id}" onclick="return confirm('Você tem certeza?')">
